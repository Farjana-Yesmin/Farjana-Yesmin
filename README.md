# Farjana Yesmin

## Researcher | Machine Learning Specialist | Trustworthy AI for Healthcare

[![Website](https://img.shields.io/badge/Website-Portfolio-green)](https://farjana-yesmin.github.io)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue)](https://www.linkedin.com/in/farjana-yesmin/)
[![Email](https://img.shields.io/badge/Email-Contact-red)](mailto:farjanayesmin76@gmail.com)
[![Google Scholar](https://img.shields.io/badge/Google_Scholar-Profile-lightgrey)](https://scholar.google.com/citations?user=AKL9avYAAAAJ&hl=en)
[![GitHub](https://img.shields.io/badge/GitHub-Repositories-black)](https://github.com/Farjana-Yesmin?tab=repositories)

---

### 👋 About Me

I'm a dedicated researcher and data scientist specializing in **Trustworthy AI**, with extensive experience developing machine learning systems that are accurate, fair, interpretable, and privacy-preserving for healthcare applications. My work bridges federated learning, fairness in AI, explainable ML, and multimodal deep learning for medical applications.

- 🔬 **Current Focus:** Privacy-preserving federated learning, intersectional fairness, interpretable AI, neuro-symbolic approaches, and multimodal healthcare analytics
- 📚 **Research Output:** Papers published/accepted at Springer LNNS, IEEE conferences, EAI proceedings, and under review at top venues (ECAF, ICMI, CIBB, ICML workshops)
- 🎖️ **Certifications:** ACM Certified Reviewer, Elsevier Certified Peer Reviewer, Nature Masterclasses: Focus on Peer Review
- 🤝 **Open to:** Research collaborations in Trustworthy AI, healthcare ML, and interdisciplinary projects; open to relocation and remote work
- 💬 **Ask me about:** Federated Learning, AI Fairness, Explainable AI, Healthcare ML, Transformers, Neuro-Symbolic AI, Multimodal Learning
- 🌍 **Languages:** Bengali (Native), English (Fluent), Norwegian (Elementary)
- ⚡ **Beyond Research:** Avid runner, pianist, and travel writer

---

### 🏆 Recent Research Highlights

| Area | Key Contributions | Status |
|------|-------------------|--------|
| **AI Fairness** | Fairness-aware representation learning for ECG-based disease prediction (wearable systems) | **Accepted (MobiHealth 2026)** |
| **Explainable AI** | Hybrid Fuzzy-XGBoost with clinician validation for maternal health risk prediction | **Accepted (ICAIHE 2026)** |
| **Fairness in Aid** | Fairness-aware AI framework for prioritizing post-flood aid | **Accepted (CCAI 2026)** |
| **Privacy-Preserving AI** | MedHE: Communication-efficient FL with adaptive gradient sparsification for healthcare | Under Review (CIBB 2026) |
| **Medical Imaging** | Joint denoising & 3D point cloud reconstruction from single medical images | Under Review (CIBB 2026) |
| **Multimodal Interaction** | Generative AI for synthetic data generation in situation awareness training | Under Review (ICMI 2026) |
| **Multimodal Healthcare** | Explainable fall detection for Parkinson's using wearable sensors | Under Review (ICMI 2026) |
| **Interpretable RAG** | Beyond the Black Box: Framework for interpretable retrieval-augmented generation | Under Review (ECAF'26) |
| **Intersectional Fairness** | Diagnostic explainability for intersectional bias in image classification | Under Review (ECAF'26) |
| **Trustworthy Healthcare** | Clinician-validated hybrid XAI for maternal health risk assessment | Under Review (HeMAI@ICMI 2026) |
| **Neuro-Symbolic AI** | Epistemic diagnosis of predicate grounding collapse in neuro-symbolic fraud detection | Under Review (EIML@ICML 2026) |

---

### 📚 Recent Publications

**Accepted Papers (Forthcoming):**
- **Yesmin, F.** , Shirmin, N. (2026). "Fairness-Aware Representation Learning for ECG-Based Disease Prediction in Wearable Systems." *Accepted at 13th EAI MobiHealth 2026, Crete, Greece. Springer/EAI proceedings.*
- **Yesmin, F.** , Shirmin, N., Bristy, S.S. (2026). "Explainable AI for Maternal Health Risk Prediction in Bangladesh: A Hybrid Fuzzy-XGBoost Framework with Clinician Validation." *Accepted at ICAIHE 2026, Waseda University, Tokyo.*
- **Yesmin, F.** , Akter, R. (2026). "Toward Equitable Recovery: A Fairness-Aware AI Framework for Prioritizing Post-Flood Aid in Bangladesh." *Accepted at CCAI 2026, Nanjing, China. IEEE proceedings.*
- **Yesmin, F.** (2026). "AI Chatbots for Dengue Symptom Triage in Bangladesh." *Springer LNNS (DASGRI 2026)*
- **Yesmin, F.** (2026). "Optimized Hybrid ML for Real-Time Air Quality Prediction." *Springer LNNS (DASGRI 2026)*
- **Yesmin, F.** , Shirmin, N. (2026). "Evolving Health Indicators in Bangladesh." *Springer LNNS (DASGRI 2026)*
- **Yesmin, F.** (2025). "Cross-Domain Evaluation for Multi-Task Learning in NLP." *Artificial Intelligence eJournal*

**Papers Under Peer Review:**
- 5th European Conference on Algorithmic Fairness (ECAF'26) – two papers
- CIBB 2026 Special Session on ML for Structured Data – two papers
- 28th ACM International Conference on Multimodal Interaction (ICMI 2026) – two papers
- HeMAI Workshop @ ICMI 2026 – one paper
- 2nd Workshop on Epistemic Intelligence in ML (EIML) @ ICML 2026 – one paper

**View my complete publication list:**  
[🔗 Full Publications on My Website](https://farjana-yesmin.github.io#publications)

---

### 🛠️ Technical Expertise

**🤖 Machine Learning & AI**
- Deep Learning • Federated Learning • Fairness & Interpretability • Multimodal Learning
- Transformers • Representation Learning • Privacy-Preserving AI • Neuro-Symbolic AI
- Network Theory • Information Theory • Attention Mechanisms

**💻 Technologies & Tools**
- **ML Frameworks:** PyTorch, TensorFlow, Scikit-learn, Splunk
- **Programming:** Python, R, Java, C++, MySQL
- **Data Visualization:** Tableau, MATLAB
- **DevOps & MLOps:** Git, GitHub, Docker, Kubernetes

**🎯 Application Domains**
- Healthcare Analytics • Bioinformatics • Wearable Systems • Medical Imaging
- Natural Language Processing • Time Series Analysis • IoT Systems • Biosignal Processing
- Crisis Economics Forecasting • Synthetic Data Generation • Maternal Health Risk Assessment

---

### 🎓 Academic Service

**ACM Certified Reviewer:**
- Added to reviewer pools for: *ACM Transactions on Computing for Healthcare (HEALTH)*, *ACM Transactions on Intelligent Systems and Technology (TIST)*, *ACM Journal on Responsible Computing (ACMJRC)*, and *ACM Transactions on Multimedia Computing, Communications, and Applications (TOMM)*

**Technical Program Committee Member:**
- DravidianLangTech 2026 & LTEDI 2026 at ACL 2026 (62nd Annual Meeting of the Association for Computational Linguistics)

**Conference & Workshop Reviewer:**
- Reviewer, 2nd Workshop on Epistemic Intelligence in Machine Learning (EIML@ICML 2026), Seoul, South Korea
- Reviewer, 28th ACM International Conference on Multimodal Interaction (ICMI 2026), Napoli, Italy
- Reviewer, IndabaX Nigeria 2026 Conference (PMLR Volume 319)
- Reviewer, International Conference on Innovations in Computing, Communication, and Sustainable Technologies (ICICST 2026)
- PeerJ Computer Science (2026)
- 27th ACM International Conference on Multimodal Interaction (ICMI 2025)
- 2nd AI for Math Workshop @ ICML 2025
- ICML 2025 Workshop on Machine Unlearning for Generative AI (MUGen)
- Deep Learning Indaba Ideathon 2025
- ACM IUI 2026 (Intelligent User Interfaces) - Posters Track

---

### 🎓 Certifications

- **ACM Certified Reviewer** – Association for Computing Machinery, Jan 2026
- **Elsevier Certified Peer Reviewer** – Elsevier Researcher Academy, Jan 2026
- **Nature Masterclasses: Focus on Peer Review** – Nature Masterclasses Online Course (3.5 hours), Jan 2026
- Google Data Analytics Professional Certificate
- IBM Data Science Professional Certificate
- IBM Applied AI Professional Certificate

---

### 📊 GitHub Activity

![Farjana's GitHub stats](https://github-readme-stats.vercel.app/api?username=Farjana-Yesmin&show_icons=true&theme=radical&hide_title=true)

[![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=Farjana-Yesmin&layout=compact&theme=radical)](https://github.com/Farjana-Yesmin)

---

### 📫 Let's Connect

I'm always open to discussing research collaborations, project ideas, or opportunities in trustworthy AI and healthcare machine learning.

- **Website & Publications:** [farjana-yesmin.github.io](https://farjana-yesmin.github.io)
- **Email:** [farjanayesmin76@gmail.com](mailto:farjanayesmin76@gmail.com)
- **LinkedIn:** [Farjana Yesmin](https://www.linkedin.com/in/farjana-yesmin/)
- **Google Scholar:** [Farjana Yesmin](https://scholar.google.com/citations?user=AKL9avYAAAAJ&hl=en)
- **GitHub:** [Farjana-Yesmin](https://github.com/Farjana-Yesmin)

---

*Last updated: April 2026*
